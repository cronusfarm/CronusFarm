...
GET  /api/channel/timeline/batch?device_id=...&channels=led_a1,pump_a1,...&hours=24&rolling=0|1  JSON: { channels, day_window? }
            ...
            if path in ("/api/tele/last", "/api/status/last"):
                ...
                with lock:
                    ...
                    if path == "/api/tele/last":
                        body = query_tele_last(cur, device_id)
                    else:
                        body = query_status_last(cur, device_id)
                raw = json.dumps(body, ensure_ascii=False).encode("utf-8")
                self.send_response(200)
                self._cors_headers()
                self.send_header("Content-Type", "application/json; charset=utf-8")
    ...
    while True:
        ...
        try:
            ...
            if not need_stale and not need_skew:
                continue
            now = time.time()
            if now - _auto_rtc_last_attempt.get(device_id, 0.0) < cooldown:
                continue
            _auto_rtc_last_attempt[device_id] = now
            rtc14 = time.strftime("%Y%m%d%H%M%S", time.localtime())
            mqtt_st = _mqtt_publish_cmd(device_id, f"rtc_local={rtc14}")
            reason = "tele_stale" if need_stale else "skew"
            print(
                f"[auto_rtc] {reason} tele_stale={tele_stale}s skew={skew}s mqtt={mqtt_st} rtc={rtc14}",
                flush=True,
            )
        except Exception as e:
