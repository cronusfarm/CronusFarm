...
# 최초 1회 Pi에서 직접 실행: sudo bash pi-initial-setup.sh
#
# NOTE:
# - 기존에 잘못 추가된 파일명(pi-initial-setup.sh.sh)이 있을 수 있어,
#   배포 자동화는 이 파일명을 기준으로 호출합니다.
...
log "Mosquitto 설치..."
apt-get install -y mosquitto mosquitto-clients sqlite3
