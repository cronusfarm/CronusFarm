...
if [[ -f "$CFG" ]]; then
  ...
  # Google → callback 교차 사이트 복귀 시 CSRF 쿠키 누락(403) 방지
  if grep -q '^cookie_secure = true' "$CFG"; then
    sudo sed -i 's/^cookie_samesite = "lax"/cookie_samesite = "none"/' "$CFG" 2>/dev/null || true
    grep -q '^cookie_samesite' "$CFG" || sudo sed -i '/^cookie_secure/a cookie_samesite = "none"' "$CFG"
  fi
